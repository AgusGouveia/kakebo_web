...
def modificar(id):
    if request.method == 'GET':
        ...
        if formulario.validate():
            try:
                modificaTablaSQL("UPDATE movimientos SET fecha = ?, concepto = ?, categoria = ?, esGasto = ?, cantidaad = ? WHERE id = ?",
                ...
                flash("Modificacion realizada con éxito", "aviso")
                return redirect(url_for("index"))
            except sqlite3.Error as e:
                print("Error en update:", e)
                flash("Se ha producido un error en acceso a base de datos. Contacte con su administrador", "error")
                return render_template('modificar.html', form=formulario)
        else:
            return render_template('modificar.html', form=formulario)
